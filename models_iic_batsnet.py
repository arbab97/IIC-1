...
class ClusterIIC(object):
    ...
    def __iic_loss(self, pi_x, pi_gx):
        ...
        pi_x = tf.tile(pi_x, [self.num_repeats] + [1]
                       * len(pi_x.shape.as_list()[1:]))
        ...
        p = tf.clip_by_value(p, clip_value_min=1e-6,
        ...
        # get marginals
        pi = tf.broadcast_to(tf.reshape(
            tf.reduce_sum(p, axis=0), (k, 1)), (k, k))
        pj = tf.broadcast_to(tf.reshape(
            tf.reduce_sum(p, axis=1), (1, k)), (k, k))

        # complete the loss
        loss = -tf.reduce_sum(p * (tf.math.log(p) -
                                   tf.math.log(pi) - tf.math.log(pj)))
    ...
    def __build(self, x, gx, graph):
        ...
        # construct losses
        self.loss_A = self.__head_loss(
            z_x, z_gx, self.k_A, self.num_A_sub_heads, 'A')
        self.loss_B = self.__head_loss(
            z_x, z_gx, self.k_B, self.num_B_sub_heads, 'B')
    ...
    def plot_learning_curve(self, epoch):
        ...
        self.ax_learn[0].plot(t, self.perf['loss_A'][:epoch],
                              label='Head A', color=c['Head A'])
        self.ax_learn[0].plot(t, self.perf['loss_B'][:epoch],
                              label='Head B', color=c['Head B'])
        ...
        self.ax_learn[1].plot(t, self.perf['class_err_avg']
                              [:epoch], color=c['Head B'])
        ...
        self.fig_learn.subplots_adjust(
            left=0.1, bottom=0.15, right=0.95, top=0.95, wspace=0.25, hspace=0.3)
            ...
            for i in range(num_epochs):
                ...
                while True:
                    try:
                        ...
                        # print update
                        print('\rEpoch {:d}, Loss = {:.4f}'.format(
                            epoch, losses[i_train]), end='')
